...
#region Events
        ...
        public delegate void MapQueryHandler(Data.FeatureDataTable data, SharpMap.Geometries.BoundingBox bb);
        /// <summary>
        ...
        // ghelo
        [Description("Tolerance value for querying features.")]
        [DefaultValue(0)]
                    ...
                    else if (m_ActiveTool == Tools.Pan)
                    {
                        ...
                        else
                        {
                            ...
                            */
                            ...
                            // start emulate Tools.Query behaviour
                            ...
                            if (m_Map.Layers.Count > m_QueryLayerIndex && m_QueryLayerIndex > -1)
                            {
                                if (m_Map.Layers[m_QueryLayerIndex] is Layers.ICanQueryLayer)
                                {
                                    ...
                                    // start ghelo
                                    if (m_Tolerance > 0)
                                        bounding = bounding.Grow(m_Tolerance);

                                    //CoordinateSystems.Transformations.GeometryTransform.TransformBox(bounding,

                                    //if (m_Map.Layers[m_QueryLayerIndex] is SharpMap.Layers.VectorLayer)
                                    //{
                                    //    if ((m_Map.Layers[m_QueryLayerIndex] as SharpMap.Layers.VectorLayer).CoordinateTransformation != null)
                                    //    {

                                    //        bounding = SharpMap.CoordinateSystems.Transformations.GeometryTransform.TransformBox(bounding, (m_Map.Layers[m_QueryLayerIndex] as SharpMap.Layers.VectorLayer).CoordinateTransformation.MathTransform.Inverse());
                                    //    }
                                    //}
                                    ...
                                    if (ds.Tables.Count > 0)
                                        if (MapQueried != null) MapQueried(ds.Tables[0], bounding);
                                        else if (MapQueried != null) MapQueried(new Data.FeatureDataTable(), bounding);
                                }
                            ...
                            // end emulate Tools.Query behaviour

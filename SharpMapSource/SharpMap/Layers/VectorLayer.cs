...
namespace SharpMap.Layers
{
    ...
    public class VectorLayer : Layer, ICanQueryLayer, IDisposable
    {
        ...
        public VectorLayer CloneLayer()
        {
            VectorLayer vl = new VectorLayer(LayerName);
            vl.ClippingEnabled = ClippingEnabled;
            vl.CoordinateTransformation = CoordinateTransformation;
            vl.DataSource = DataSource;
            vl.Enabled = Enabled;
            vl.MaxVisible = MaxVisible;
            vl.MinVisible = MinVisible;
            vl.SmoothingMode = SmoothingMode;
            vl.SRID = SRID;
            vl.Style = Style.Clone();
            vl.Theme = Theme;
                ...
                if (Style.EnableOutline)
                {
                    ...
                    for (int i = 0; i < features.Count; i++)
                    {
                        ...
                        if (feature.Geometry is LineString)
                        {
                            VectorStyle outlinestyle1 = Theme.GetStyle(feature) as VectorStyle;
                            if (outlinestyle1.Enabled && outlinestyle1.EnableOutline)
                                XGraphicsVectorRenderer.DrawLineString(g, feature.Geometry as LineString, outlinestyle1.Outline,
                                                              map);
                        }
                        else if (feature.Geometry is MultiLineString)
                        {
                            VectorStyle outlinestyle2 = Theme.GetStyle(feature) as VectorStyle;
                            if (outlinestyle2.Enabled && outlinestyle2.EnableOutline)
                                XGraphicsVectorRenderer.DrawMultiLineString(g, feature.Geometry as MultiLineString,
                                                                   outlinestyle2.Outline, map);
                        }
                    }
                }
                ...
                if (Style.EnableOutline)
                {
                    foreach (Geometry geom in geoms)
                    {
                        if (geom != null)
                        {
                            ...
                            {
                                case "SharpMap.Geometries.LineString":
                                    XGraphicsVectorRenderer.DrawLineString(g, geom as LineString, Style.Outline, map);
                                    break;
                                case "SharpMap.Geometries.MultiLineString":
                                    XGraphicsVectorRenderer.DrawMultiLineString(g, geom as MultiLineString, Style.Outline, map);
                                    break;
                                default:
                                    break;
                            }
        ...
        private void RenderGeometry(XGraphics g, Map map, Geometry feature, VectorStyle style)
        {
            ...
            //switch (feature.GetType().FullName)
            {
                case GeometryType2.Polygon:
                    //case "SharpMap.Geometries.Polygon":
                    if (style.EnableOutline)
                        XGraphicsVectorRenderer.DrawPolygon(g, (Polygon)feature, style.Fill, style.Outline, _ClippingEnabled,
                                                   map);
                    else
                        XGraphicsVectorRenderer.DrawPolygon(g, (Polygon)feature, style.Fill, null, _ClippingEnabled, map);
                    break;
                case GeometryType2.MultiPolygon:
                    //case "SharpMap.Geometries.MultiPolygon":
                    if (style.EnableOutline)
                        XGraphicsVectorRenderer.DrawMultiPolygon(g, (MultiPolygon)feature, style.Fill, style.Outline,
                                                        _ClippingEnabled, map);
                    else
                        XGraphicsVectorRenderer.DrawMultiPolygon(g, (MultiPolygon)feature, style.Fill, null, _ClippingEnabled,
                                                        map);
                    break;
                case GeometryType2.LineString:
                    //case "SharpMap.Geometries.LineString":
                    XGraphicsVectorRenderer.DrawLineString(g, (LineString)feature, style.Line, map);
                    break;
                case GeometryType2.MultiLineString:
                    //case "SharpMap.Geometries.MultiLineString":
                    XGraphicsVectorRenderer.DrawMultiLineString(g, (MultiLineString)feature, style.Line, map);
                    break;
                case GeometryType2.Point:
                    //case "SharpMap.Geometries.Point":
                    XGraphicsVectorRenderer.DrawPoint(g, (Point)feature, style.Symbol, style.SymbolScale, style.SymbolOffset,
                                             style.SymbolRotation, map);
                    break;
                case GeometryType2.MultiPoint:
                    //case "SharpMap.Geometries.MultiPoint":
                    XGraphicsVectorRenderer.DrawMultiPoint(g, (MultiPoint)feature, style.Symbol, style.SymbolScale,
                                                  style.SymbolOffset, style.SymbolRotation, map);
                    break;

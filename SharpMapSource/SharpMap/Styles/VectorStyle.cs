...
namespace SharpMap.Styles
{
    ...
    public class VectorStyle : Style
    {
        ...
        internal VectorStyle Clone()
        {
            VectorStyle vs = new VectorStyle();

            vs.Enabled = Enabled;
            vs.EnableOutline = EnableOutline;
            vs.Fill = Fill.Clone() as Brush;
            vs.Line = Line.Clone() as Pen;
            vs.MaxVisible = MaxVisible;
            vs.MinVisible = MinVisible;
            vs.Outline = Outline.Clone() as Pen;

            if (Symbol != null)
                vs.Symbol = Symbol.Clone() as Bitmap;
            else
                vs.Symbol = null;

            vs.SymbolOffset = SymbolOffset;
            vs.SymbolRotation = SymbolRotation;
            vs.SymbolScale = SymbolScale;
